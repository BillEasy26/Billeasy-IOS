...
/// Aqui eu transformo a área de empresas em um CRUD real, mantendo fallback local quando o backend não estiver disponível.
final class CompaniesViewController: UITableViewController, UISearchResultsUpdating {
    private let session: AuthSession
    private let directoryService: PortalDirectoryService
    private let actionsService: PortalActionsService

    private var companies: [PortalCompanyRecord] = []
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(
            ...
            style: .plain,
            target: self,
            action: #selector(addCompanyTapped)
        )
        navigationItem.rightBarButtonItem?.accessibilityIdentifier = "companies.addButton"
        ...
        loadCompanies(showErrorToast: false)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        config.secondaryText = makeCompanySecondaryText(company)
        ...
        cell.backgroundConfiguration = makeCellBackgroundConfiguration(isActive: company.isActive)
        cell.accessoryType = .none
        cell.accessoryView = makeCompanyAccessoryView(company)
        cell.selectionStyle = .none
        cell.accessibilityIdentifier = "companies.card.\(company.id)"
        cell.accessibilityLabel = company.name
        cell.accessibilityValue = makeCompanyAccessibilityValue(company)
        return cell
    }

    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        presentCompanyForm(mode: .edit(companies[indexPath.row]))
    }
    ...
    override func tableView(
        ...
        let company = companies[indexPath.row]
        guard company.isActive else { return nil }

        let action = UIContextualAction(style: .destructive, title: "Inativar") { [weak self] _, _, completion in
            self?.performCompanyLifecycle(
                company: company,
                action: .suspend,
                successMessage: "Empresa inativada com sucesso.",
                completion: completion
            )
        }
        action.backgroundColor = UIColor(hex: "#D97706")
    ...
    override func tableView(
        ...
        let company = companies[indexPath.row]
        guard company.isActive == false else { return nil }

        let action = UIContextualAction(style: .normal, title: "Ativar") { [weak self] _, _, completion in
            self?.performCompanyLifecycle(
                company: company,
                action: .activate,
                successMessage: "Empresa ativada com sucesso.",
                completion: completion
            )
        }
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        searchTask = Task { [weak self] in
            ...
            await MainActor.run {
                self?.loadCompanies(showErrorToast: false)
            }
        }
    }

    @objc private func addCompanyTapped() {
        presentCompanyForm(mode: .create)
    }

    private func loadCompanies(showErrorToast: Bool) {
        loadTask?.cancel()
        isLoading = true
        ...
        loadTask = Task { [weak self] in
            ...
            do {
                let result = try await directoryService.listCompanies(
                    session: session,
    ...
    private func presentCompanyForm(mode: CompanyFormMode) {
        let controller = CompanyFormViewController(
            session: session,
            mode: mode,
            directoryService: directoryService,
            actionsService: actionsService
        )
        controller.onSave = { [weak self] company in
            guard let self else { return }
            switch mode {
            case .create:
                self.showSimpleToast("Empresa cadastrada com sucesso.", style: .success)
                self.companies.insert(company, at: 0)
            case .edit:
                self.showSimpleToast("Empresa atualizada com sucesso.", style: .success)
            ...
            self.companies.sort { $0.name.localizedCaseInsensitiveCompare($1.name) == .orderedAscending }
            self.updateBackgroundState()
            self.tableView.reloadData()
        }
    ...
    private func performCompanyLifecycle(
        company: PortalCompanyRecord,
        action: PortalCompanyLifecycleAction,
        ...
        lifecycleTask = Task { [weak self] in
            ...
            do {
                let updatedCompany = try await directoryService.updateCompanyLifecycle(
                    companyID: company.id,
                    action: action
                )

                await MainActor.run {
                    if let index = self.companies.firstIndex(where: { $0.id == updatedCompany.id }) {
    ...
    private func updateBackgroundState() {
        if isLoading {
            let card = BrandCardFactory.makeLoadingStateCard(
                ...
                subtitle: "Estou atualizando a lista e sincronizando os dados do backend."
            )
        ...
        let card = BrandCardFactory.makeEmptyStateCard(
            ...
            subtitle: "Use o botão + para cadastrar uma empresa e manter o cadastro alinhado com o backend e com o app.",
    ...
    private func makeCompanySecondaryText(_ company: PortalCompanyRecord) -> String {
        let firstLine = [
            Formatters.formatCPFOrCNPJ(company.document).nilIfEmpty,
            formattedPhone(company.phone).nilIfEmpty
        ].compactMap { $0 }.joined(separator: " • ")

        return [
            firstLine.nilIfEmpty,
            companyTypeTitle(company.type),
            company.addressSummary?.nilIfEmpty,
            company.responsibleName?.nilIfEmpty.map { "Responsável: \($0)" }
        ]
        .compactMap { $0 }
        .joined(separator: "\n")
    }

    private func makeCompanyAccessibilityValue(_ company: PortalCompanyRecord) -> String {
        [
            Formatters.formatCPFOrCNPJ(company.document).nilIfEmpty,
            formattedPhone(company.phone).nilIfEmpty,
            companyTypeTitle(company.type),
            companyStatusTitle(company.status),
            company.addressSummary?.nilIfEmpty
        ]
        .compactMap { $0 }
        .joined(separator: ", ")
    ...
    private func makeCompanyAccessoryView(_ company: PortalCompanyRecord) -> UIView {
        let stack = UIStackView()
        stack.axis = .horizontal
        stack.alignment = .center
        stack.spacing = 8

        let badge = makeCompanyStatusBadge(company)
    ...
    private func makeCompanyStatusBadge(_ company: PortalCompanyRecord) -> UIView {
        ...
        let style = companyStatusStyle(company.status)
    ...
    private func companyStatusStyle(_ rawValue: String?) -> (title: String, backgroundColor: UIColor, borderColor: UIColor, textColor: UIColor) {
        ...
        default:
            return (companyStatusTitle(rawValue) ?? "Status", UIColor(hex: "#E8F2FA"), UIColor(hex: "#C8D6E8"), UIColor(hex: "#1579A8"))
        }
    }

    private func companyTypeTitle(_ rawValue: String?) -> String? {
        switch rawValue?.uppercased() {
        case "PESSOA_FISICA": return "Pessoa Física"
        case "PESSOA_JURIDICA": return "Pessoa Jurídica"
        default: return rawValue?.nilIfEmpty
        }
    }

    private func companyStatusTitle(_ rawValue: String?) -> String? {
        switch rawValue?.uppercased() {
    ...
    private static let cellReuseIdentifier = "company_cell"
}

private enum CompanyFormMode {
    case create
    case edit(PortalCompanyRecord)
}

private final class CompanyFormViewController: UIViewController, UITextFieldDelegate {
    private enum CompanyTypeOption: String, CaseIterable {
        ...
        var title: String {
            ...
            case .pessoaJuridica: return "Pessoa Jurídica"
            }
        }
    }

    private let session: AuthSession
    private let mode: CompanyFormMode
    private let directoryService: PortalDirectoryService
    private let actionsService: PortalActionsService

    var onSave: ((PortalCompanyRecord) -> Void)?
    ...
    private let typeControl = UISegmentedControl(items: CompanyTypeOption.allCases.map(\.title))
    private let cepField = UITextField()
    private let numberField = UITextField()
    private let complementField = UITextField()
    private let pixKeyField = UITextField()
    ...
    private var selectedType: CompanyTypeOption = .pessoaJuridica
    ...
    private func setupView() {
        ...
        let introCard = BrandCardFactory.makeEmptyStateCard(
            title: title ?? "Empresa",
            subtitle: "Eu sigo o mesmo contrato do backend web para nome, documento, telefone e endereço com CEP.",
            iconSystemName: "building.2"
        )
        stackView.addArrangedSubview(introCard)
        stackView.addArrangedSubview(makeSectionCard(title: "Dados da Empresa", rows: [
            ...
            makeLabeledField(title: "Tipo", field: typeControl)
        ]))
        stackView.addArrangedSubview(makeAddressCard())
        stackView.addArrangedSubview(makeSectionCard(title: "PIX (Opcional)", rows: [
            makeLabeledField(title: "Chave PIX", field: pixKeyField)
        ]))

        configureTextField(nameField, placeholder: "Nome da empresa", keyboardType: .default, identifier: "companies.form.name")
        configureTextField(documentField, placeholder: "00.000.000/0000-00", keyboardType: .numberPad, identifier: "companies.form.document")
        configureTextField(phoneField, placeholder: "(00) 00000-0000", keyboardType: .numberPad, identifier: "companies.form.phone")
        configureTextField(cepField, placeholder: "00000-000", keyboardType: .numberPad, identifier: "companies.form.cep")
        configureTextField(numberField, placeholder: "1578", keyboardType: .default, identifier: "companies.form.number")
        configureTextField(complementField, placeholder: "Sala 12, Apto 3", keyboardType: .default, identifier: "companies.form.complement")
        configureTextField(pixKeyField, placeholder: "Opcional", keyboardType: .default, identifier: "companies.form.pixKey")
        ...
        typeControl.selectedSegmentIndex = CompanyTypeOption.allCases.firstIndex(of: selectedType) ?? 1
        typeControl.backgroundColor = UIColor(hex: "#F7FAFD")
        typeControl.selectedSegmentTintColor = UIColor(hex: "#2E87C8")
        typeControl.setTitleTextAttributes([
            .foregroundColor: UIColor(hex: "#283344"),
            .font: UIFont.billeasyScaledFont(size: 13, weight: .bold, textStyle: .subheadline)
        ], for: .normal)
        typeControl.setTitleTextAttributes([
        ...
        cepLookupButton.titleLabel?.numberOfLines = 1
    ...
    private func populateIfNeeded() {
        guard case let .edit(company) = mode else { return }
        nameField.text = company.name
        documentField.text = Formatters.formatCPFOrCNPJ(company.document)
        phoneField.text = formatPhone(company.phone)
        pixKeyField.text = company.pixKey
        if let foundType = CompanyTypeOption(rawValue: company.type.uppercased()) {
    ...
    @objc private func saveTapped() {
        ...
        saveTask = Task { [weak self] in
            ...
            do {
                let saved: PortalCompanyRecord
                switch mode {
                case .create:
                    saved = try await directoryService.createCompany(session: session, input: input)
                case let .edit(company):
                    saved = try await directoryService.updateCompany(companyID: company.id, input: input)
                }
    ...
    private func currentFormInput() -> PortalCompanyFormInput {
        PortalCompanyFormInput(
            name: nameField.text ?? "",
            document: documentField.text ?? "",
            phone: phoneField.text ?? "",
            type: CompanyTypeOption.allCases[typeControl.selectedSegmentIndex].rawValue,
            cep: cepField.text?.nilIfEmpty,
            number: numberField.text?.nilIfEmpty,
            complement: complementField.text?.nilIfEmpty,
            pixKey: pixKeyField.text?.nilIfEmpty,
            pixKeyType: nil
        )
    }

    private func validate(_ input: PortalCompanyFormInput) -> String? {
        if input.name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "Informe o nome da empresa."
        }
        if Formatters.digitsOnly(input.document).count < 11 {
            return "Informe um CPF/CNPJ válido da empresa."
        }
        if Formatters.digitsOnly(input.phone).count < 10 {
            return "Informe um telefone válido da empresa."
        }

        if case .edit = mode {
            guard Formatters.digitsOnly(input.cep ?? "").count == 8 else {
                return "Informe um CEP válido para salvar a empresa."
            }
            if input.number?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty != false {
                return "Informe o número do endereço da empresa."
            }
        }

        if let cep = input.cep, cep.isEmpty == false, Formatters.digitsOnly(cep).count != 8 {
            return "Informe um CEP válido para a empresa."
        }
        if let number = input.number, input.cep?.isEmpty == false, number.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "Informe o número do endereço da empresa."
        }
    ...
    private func formatPhone(_ value: String) -> String {
        let digits = Formatters.digitsOnly(value)
        if digits.count <= 10 {
            return CompaniesViewController.formatPhone10(digits)
        }
        return CompaniesViewController.formatPhone11(digits)
    }
}
